...
export function MyLoans({ onRequestLoan }: MyLoansProps) {
  ...
  const stats = {
    ...
    completed: loans?.filter((l) => l.status === LoanStatus.Completed).length || 0,
    defaulted: loans?.filter((l) => l.status === LoanStatus.Defaulted).length || 0,
    totalBorrowed:
      loans
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold">{formatCurrency(stats.totalBorrowed)}</p>
        </Card>
        <Card className="p-4">
          <p className="text-sm text-gray-600">Total Repaid</p>
          <p className="text-2xl font-bold text-blue-600">{formatCurrency(stats.totalRepaid)}</p>
        </Card>
      </div>
              ...
              {count > 0 && (
                <span
                  className={`ml-2 px-2 py-0.5 rounded-full text-xs ${
                    filter === key ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'
                  }`}
                >
                  {count}
                </span>
      ...
      {activeLoanCount >= 3 && (
        <Card className="p-4 bg-yellow-50 border border-yellow-200">
          <p className="text-sm text-yellow-800">
            ⚠️ You have reached the maximum of 3 active loans. Complete or pay off existing
            loans to request new ones.
          </p>
        </Card>
      )}
    </div>
