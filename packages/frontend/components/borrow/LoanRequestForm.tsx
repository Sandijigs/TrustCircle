...
export function LoanRequestForm({
  ...
  const { creditScore: creditScoreData, isLoading: creditLoading } =
  ...
  const [calculatedTerms, setCalculatedTerms] = useState({
    ...
    totalPayment: 0,
    apr: 0,
    maxLoanAmount: maxLoanAmountForScore, // Initialize with actual value, not 0!
  });
  ...
  useEffect(() => {
    ...
    const interestRateBPS = calculateInterestRate(
      score,
      formData.hasCollateral
    );

    // If no amount, just set the max loan amount
    if (!formData.amount || isNaN(Number(formData.amount))) {
      setCalculatedTerms({
        ...
        totalPayment: 0,
        apr: 0,
        maxLoanAmount: maxLoanAmountForScore,
      });
      return;
    }
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.amount || isNaN(amount)) {
        ...
        newErrors.amount = `Minimum loan amount is ${formatCurrency(
          MIN_LOAN_AMOUNT
        )}`;
      } else if (amount > MAX_LOAN_AMOUNT) {
        newErrors.amount = `Maximum loan amount is ${formatCurrency(
          MAX_LOAN_AMOUNT
        )}`;
      } else if (amount > calculatedTerms.maxLoanAmount) {
        newErrors.amount = `Your credit score allows up to ${formatCurrency(
          calculatedTerms.maxLoanAmount
        )}`;
      }
    ...
    if (currentStep === 2) {
      if (formData.durationMonths < daysToMonths(MIN_DURATION_DAYS)) {
        newErrors.duration = `Minimum duration is ${daysToMonths(
          MIN_DURATION_DAYS
        )} months`;
      } else if (formData.durationMonths > daysToMonths(MAX_DURATION_DAYS)) {
        newErrors.duration = `Maximum duration is ${daysToMonths(
          MAX_DURATION_DAYS
        )} months`;
      }
    }
          ...
          <div className="space-y-6">
            ...
            <div className="text-gray-600">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Your credit score allows up to{" "}
                {formatCurrency(calculatedTerms.maxLoanAmount)}
              </p>
            </div>
          ...
          <div className="space-y-6">
                  ...
                  ([key, label]) => (
                    <button
                      ...
                      onClick={() =>
                        setFormData({
                          ...formData,
                          frequency: Number(key) as PaymentFrequency,
                        })
                      }
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">
                  {formatCurrency(calculatedTerms.totalPayment)}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Number of Installments:</span>
                <span className="font-semibold">
                  {calculatedTerms.totalInstallments}
                </span>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex items-center">
                ...
                <label htmlFor="hasCollateral" className="ml-3">
                  <span className="font-semibold">
                    Add collateral to my loan
                  </span>
                  <p className="text-sm text-gray-600">
                    Reduce interest rate from{" "}
                    {formatBPS(
                      calculateInterestRate(
                        creditScoreData?.score || 500,
                        false
                    ...
                    to{" "}
                    {formatBPS(
                      calculateInterestRate(creditScoreData?.score || 500, true)
                    )}
                  </p>
                </label>
              </div>
            ...
            {formData.hasCollateral && (
              <div className="space-y-4 p-4 border-2 border-blue-200 rounded-lg">
                <p className="text-sm text-gray-600">
                  Collateral management will be available in the next step.
                  You'll be able to deposit ERC20 tokens or NFTs after loan
                  approval.
                </p>
              </div>
            )}
          ...
          <div className="space-y-6">
              ...
              <div className="pt-4 border-t">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {calculatedTerms.interestRate.toFixed(2)}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Total Interest:</span>
            ...
            <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
              ...
              <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
                ...
                {creditScoreData?.score && creditScoreData.score >= 800 && (
                  <li className="text-green-700 font-semibold">
                    ✓ Your loan will be auto-approved due to excellent credit
                    score
                  </li>
                )}
              </ul>
